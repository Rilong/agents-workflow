...
Concrete templates for the most common building blocks. All files declare `declare(strict_types=1)` and use full type hints.

### Form Request (§6)

All form submissions require a Form Request — never validate in the Action/controller body.
...
Validation errors flow automatically: Form Request → `$page.props.errors` (Inertia) → `useForm().errors.field` (Vue).
...
Naming: `create_posts_table`, `add_slug_to_posts_table`, `drop_legacy_field_from_users_table`. Never modify a migration that has already run — always create a new one. Generate with `php artisan make:migration`.

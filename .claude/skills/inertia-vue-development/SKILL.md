...
These project-specific rules override the generic examples below — apply them on every new component:

- **TypeScript** — hybrid stack migrating to TS; use `<script setup lang="ts">` on all new components. Never use the Options API.
- **Ziggy routes** — never hardcode URLs. Use the `route()` helper from `ziggy-js` (see the `ziggy-development` skill). Example: `form.post(route('posts.store'))`, not `form.post('/posts')`.
- **Shared props** — access shared props (from `HandleInertiaRequests`) via `usePage()`. Never re-pass them as component props:
  ```typescript
  const page = usePage<{ auth: { user: User }; flash: Flash }>()
  const user = computed(() => page.props.auth.user)
  ```
- **Form Requests** — Laravel Form Request validation errors map automatically to `form.errors.field`. Validate in the Form Request, never in the Action body.
- **Redirects (server)** — `redirect()->route('posts.index')` for Inertia redirects; `Inertia::location(route('posts.index'))` for external / full-page redirects.

### SSR Caveats

- No `window` / `document` / `localStorage` at the top level of `<script setup>` — wrap them in `onMounted`.
- Pinia stores must initialize server-safe (no browser APIs in state initializers).
